...
void gts_navigator::Navigator::navigate_to_pose_goal_status_subscription_cb(const action_msgs::msg::GoalStatusArray::SharedPtr goal_status_array_cb_data)
{
    ...
    else
    {
        ...
        const builtin_interfaces::msg::Time &&time_moved = std::move(*stamp);
        ...
        gts_navigation_status->set__header(header_moved);
        ...
        gts_navigation_result->set__header(header_moved);
        ...
        else
        {
            ...
            this->gts_navigation_status_publish(gts_navigation_status, RCL_NAVIGATE_TO_POSE_GOAL_ABORTED);
            return;
        }
    }
}

void gts_navigator::Navigator::gts_navigation_status_publish(const gts_navigation_msgs::msg::NavigationStatusStamped::UniquePtr &gts_navigation_status, const int &goal_status_code)
{
    bool is_gts_navigation_status_nullptr = (gts_navigation_status == nullptr);

    if (is_gts_navigation_status_nullptr)
    {
    ...
    gts_navigation_status->set__status_code(goal_status_code);
    gts_navigation_status->set__current_goal_index(slam_waypoints_list_index_);

    const gts_navigation_msgs::msg::NavigationStatusStamped &&gts_navigation_status_moved = std::move(*gts_navigation_status);
    this->gts_navigation_status_publisher_->publish(gts_navigation_status_moved);
}

void gts_navigator::Navigator::gts_navigation_result_publish(const gts_navigation_msgs::msg::NavigationResultStamped::UniquePtr &gts_navigation_result, const int &goal_status_code)
{
    bool is_gts_navigation_result_nullptr = (gts_navigation_result == nullptr);

    if (is_gts_navigation_result_nullptr)
    {
    ...
    gts_navigation_result->set__result_code(goal_status_code);
    gts_navigation_result->set__result_index(slam_waypoints_list_index_);

    const gts_navigation_msgs::msg::NavigationResultStamped &&gts_navigation_result_moved = std::move(*gts_navigation_result);
    this->gts_navigation_result_publisher_->publish(gts_navigation_result_moved);
}
...
void gts_navigator::Navigator::navigate_to_pose_send_goal()
{
    ...
    pose_stamped->set__pose(pose_moved);

    const geometry_msgs::msg::PoseStamped &&pose_stamped_moved = std::move(*pose_stamped);
    this->navigate_to_pose_goal_->set__pose(pose_stamped_moved);
